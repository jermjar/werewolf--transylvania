## NOTE - Didn't do the Lobby Chat / Messages section
extends Node
...
func leave_lobby() -> void:
	...
	if lobby_id != 0:
		...
		# Wipe the Steam lobby ID then display the default
		lobby_id = 0
		...
		for this_member in lobby_members:
			...
			if this_member['steam_id'] != steam_id:
				# Clsoe the P2P session using the Networking class
...
func _on_lobby_created(connect: int, this_lobby_id: int) -> void:
	if connect == 1:
		...
		Steam.setLobbyData(lobby_id, "mode", "Werewolf - TEST")
